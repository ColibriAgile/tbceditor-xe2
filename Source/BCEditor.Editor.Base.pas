...
type
  ...
  strict private
    ...
    procedure MoveCodeFoldingRangesAfter(AFoldRange: TBCEditorCodeFoldingRange; LineCount: Integer);
  ...
  protected
    ...
    procedure PaintCodeFolding(AClipRect: TRect; ALine: Integer);
...
begin
  ...
  for i := 0 to FAllCodeFoldingRanges.AllCount - 1 do
  with FAllCodeFoldingRanges[i] do
  if not ParentCollapsed and (ToLine = ALine) then
  begin
    Result := FAllCodeFoldingRanges[i];
    Break;
  end;
...
begin
  ...
  for i := 0 to FAllCodeFoldingRanges.AllCount - 1 do
  with FAllCodeFoldingRanges[i] do
  if not ParentCollapsed and not Collapsed and (ToLine = ALine) and Collapsable then
  begin
    Result := True;
    Break;
  end;
...
begin
  ...
  for i := 0 to FAllCodeFoldingRanges.AllCount - 1 do
  with FAllCodeFoldingRanges[i] do
  if not ParentCollapsed and not Collapsed and Collapsable and ((FromLine + 1) <= ALine) and (ToLine > ALine) then
  begin
    Result := True;
    Break;
  end;
...
begin
  ...
  begin
    if (FAllCodeFoldingRanges[i] <> LFoldRange) and not ParentCollapsed then
...
begin
  ...
  begin
    ...
    for i := 0 to LFoldRegions.Count - 1 do
    begin
      LTextPtr := PChar(UpperCase(LineText));
      ...
      begin
        ...
        //if not IsValidChar(TextPtr - 1) and (TextPtr^ = KeyWordPtr^) then
        begin
...
begin
  ...
  begin
    ...
    begin
      LStartTextPosition.Line := FromLine;
      LStartTextPosition.Char := 1;
      LEndTextPosition.Line := FromLine;
      LEndTextPosition.Char := Length(Lines[FromLine - 1]);
...
procedure TBCBaseEditor.MoveCodeFoldingRangesAfter(AFoldRange: TBCEditorCodeFoldingRange; LineCount: Integer);
var
  i, j: Integer;
begin
  i := 0;
  ...
  for j := 0 to i - 1 do
    with FAllCodeFoldingRanges[j] do
      if (not Collapsed) and (ToLine > AFoldRange.FromLine) or
        (not Collapsed) and FAllCodeFoldingRanges[j].FoldRegion.SharedClose and
        (AFoldRange.FoldRegion.CloseToken = FAllCodeFoldingRanges[j].FoldRegion.CloseToken) then
...
var
  ...
  LFoldOpenKeyChars, LFoldCloseKeyChars, LSkipOpenKeyChars, LSkipCloseKeyChars: TBCEditorCharSet;
  ...
  { We are only interested in first characters of Open and Close tokens }
  ...
  begin
    ...
    for i := 0 to j do
    begin
      Add(LSkipOpenKeyChars, PChar(Highlighter.CodeFoldingRegions.SkipRegions[i].OpenToken));
      Add(LSkipCloseKeyChars, PChar(Highlighter.CodeFoldingRegions.SkipRegions[i].CloseToken));
    end;
    j := Highlighter.CodeFoldingRegions.Count - 1;
    for i := 0 to j do
    begin
      Add(LFoldOpenKeyChars, PChar(FHighlighter.CodeFoldingRegions[i].OpenToken));
      if FHighlighter.CodeFoldingRegions[i].BreakIfNotFoundBeforeNextRegion <> '' then
        Add(LFoldOpenKeyChars, PChar(FHighlighter.CodeFoldingRegions[i].BreakIfNotFoundBeforeNextRegion));
      Add(LFoldCloseKeyChars, PChar(Highlighter.CodeFoldingRegions[i].CloseToken));
    end;
  end;
  ...
  begin
    ...
    if (LOpenTokenSkipFoldRangeList.Count > 0) and
      CharInSet(LTextPtr^, LSkipCloseKeyChars) and not OddCountOfStringEscapeChars(LTextPtr) then
    begin
    ...
    if CharInSet(LTextPtr^, LSkipOpenKeyChars) then
      ...
      begin
        ...
        begin
          ...
          { check if the open keyword found }
          while (LTextPtr^ <> BCEDITOR_NONE_CHAR) and (LKeyWordPtr^ <> BCEDITOR_NONE_CHAR) and ((LTextPtr^ = LKeyWordPtr^) or
             (Highlighter.CodeFoldingRegions.SkipRegions[i].SkipEmptyChars and
             ((LTextPtr^ = BCEDITOR_SPACE_CHAR) or (LTextPtr^ = BCEDITOR_TAB_CHAR)) )) do
          begin
            if not Highlighter.CodeFoldingRegions.SkipRegions[i].SkipEmptyChars or
              (Highlighter.CodeFoldingRegions.SkipRegions[i].SkipEmptyChars and
          ...
          begin
            if Highlighter.CodeFoldingRegions.SkipRegions[i].RegionType = ritSingleLineComment then
            { single line comment skip until next line }
              Exit(True)
            else
              LOpenTokenSkipFoldRangeList.Add(Highlighter.CodeFoldingRegions.SkipRegions[i]);
            Dec(LTextPtr); { the end of the while loop will increase }
            Break; { for i := 0 to Highlighter.CodeFoldingRegions... }
          end
          else
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            j := Highlighter.CodeFoldingRegions.Count - 1;
            for i := 0 to j do
            if Highlighter.CodeFoldingRegions[i].OpenIsClose then { optimizing... }
            begin
              if LTextPtr^ = PChar(Highlighter.CodeFoldingRegions[i].OpenToken)^ then { if first character match }
              begin
                LKeyWordPtr := PChar(Highlighter.CodeFoldingRegions[i].OpenToken);
                ...
                if LKeyWordPtr^ = BCEDITOR_NONE_CHAR then
                begin
                  if (Highlighter.CodeFoldingRegions[i].OpenTokenLength = 1) or IsWholeWord(LBookmarkTextPtr2 - 1, LTextPtr) then { not interested in partial hits }
                  begin
                    if LOpenTokenFoldRangeList.Count > 0 then
                    ...
                    LFoldRange := LFoldRanges.Add(ATopFoldRanges, LLine + 1, GetLineIndentChars(AStrings, LLine{ + 1}), LFoldCount,
                      Highlighter.CodeFoldingRegions[i], LLine + 1);
                    { open keyword found }
                    LOpenTokenFoldRangeList.Add(LFoldRange);
                    Inc(LFoldCount);
                    Break; { for i := 0 to Highlighter.CodeFoldingRegions.Count - 1 }
                  end
                  else
      ...
      begin
        ...
        begin
            ...
            if LFoldRange.FoldRegion.BreakIfNotFoundBeforeNextRegion <> '' then
              ...
              begin
                ...
                while (LTextPtr^ <> BCEDITOR_NONE_CHAR) and (LKeyWordPtr^ <> BCEDITOR_NONE_CHAR) and
                  ((LTextPtr^ = LKeyWordPtr^) or (LTextPtr^ = BCEDITOR_SPACE_CHAR) or (LTextPtr^ = BCEDITOR_TAB_CHAR)) do
                begin
          ...
          begin
            ...
            if LKeyWordPtr^ = BCEDITOR_NONE_CHAR then
            begin
              if (Highlighter.CodeFoldingRegions[i].OpenTokenLength = 1) or IsWholeWord(LBookmarkTextPtr - 1, LTextPtr) then { not interested in partial hits }
              begin
                { check if special rule found }
                LSkipIfFoundAfterOpenToken := False;
                if Highlighter.CodeFoldingRegions[i].SkipIfFoundAfterOpenToken <> '' then
                begin
                  while LTextPtr^ <> BCEDITOR_NONE_CHAR do
                  begin
                    LKeyWordPtr := PChar(Highlighter.CodeFoldingRegions[i].SkipIfFoundAfterOpenToken);
                    LBookmarkTextPtr2 := LTextPtr;
                    if LTextPtr^ = LKeyWordPtr^ then { if first character match }
                    begin
                      while (LTextPtr^ <> BCEDITOR_NONE_CHAR) and (LKeyWordPtr^ <> BCEDITOR_NONE_CHAR) and (LTextPtr^ = LKeyWordPtr^) do
                      begin
                        Inc(LTextPtr);
                ...
                //if Assigned(LFoldRange) and (LFoldRange.FoldRegion.BreakIfNotFoundBeforeNextRegion <> '') then
                //  LFoldRange.IsExtraTokenFound := False;
                ...
                Inc(LFoldCount);
                Dec(LTextPtr); { the end of the while loop will increase }
                Break; { for i := 0 to Highlighter.CodeFoldingRegions.Count }
              end
              else
                LTextPtr := LBookmarkTextPtr; { region not found, return pointer back }
            end
...
begin
  ...
  try
    ...
    begin
      LTextPtr := PChar(UpperCase(AStrings[LLine]));
...
procedure TBCBaseEditor.SearchHighlighterAfterPaint(ACanvas: TCanvas; AFirstLine, ALastLine: integer);
  ...
  begin
    ...
    begin
      ...
      FSearchHighlighterBitmap.Canvas.FillRect(Rect(0, 0, FSearchHighlighterBitmap.Width, FSearchHighlighterBitmap.Height));
...
begin
  ...
  begin
    FCharsInWindow := Max(ClientWidth - FLeftMargin.GetWidth - FCodeFolding.GetWidth - 1 - FMinimap.GetWidth -
...
begin
  for i := 0 to FAllCodeFoldingRanges.AllCount - 1 do
  if (FAllCodeFoldingRanges[i].Collapsed) and (not FAllCodeFoldingRanges[i].ParentCollapsed) then
  begin
    LNearest := -1;
    LCollapsedFromLine := FAllCodeFoldingRanges[i].FromLine;

    for j := 0 to FAllCodeFoldingRanges.AllCount - 1 do
      with FAllCodeFoldingRanges[j] do
        if (not Collapsed) and (not ParentCollapsed) and (FromLine < LCollapsedFromLine) and
          (LCollapsedFromLine < ToLine) then
          if (LNearest = -1) or (FromLine > FAllCodeFoldingRanges[LNearest].FromLine) then
            LNearest := j;

    if LNearest > -1 then
    begin
      FAllCodeFoldingRanges[i].FoldRangeLevel := Succ(FAllCodeFoldingRanges[LNearest].FoldRangeLevel);
      FAllCodeFoldingRanges[LNearest].SubFoldRanges.Ranges.Add(FAllCodeFoldingRanges[i]);
    end
    else
    begin
      j := 0;
      ...
      repeat
        if FoldRanges[j].FromLine > LCollapsedFromLine then
        begin
          Ranges.Insert(j, FAllCodeFoldingRanges[i]);
          Break;
        end;
      ...
      until j >= Count - 1
        ...
        Ranges.Add(FAllCodeFoldingRanges[i]);
    end;
  end;
end;
...
begin
  ...
  begin
    if FromLine > ACurrentLine then
    begin
      MoveBy(ALineCount);

      if Collapsed then
        UpdateFoldRanges(SubFoldRanges, ALineCount);
    ...
    if FromLine = ACurrentLine then
    begin
      LPosition := Pos(FoldRegion.OpenToken, UpperCase(Lines[FromLine]));
    ...
    begin
      if ToLine > ACurrentLine then
        Widen(ALineCount)
      else
      if ToLine = ACurrentLine then
      begin
        LPosition := Pos(FoldRegion.CloseToken, UpperCase(Lines[ToLine]));
...
begin
  ...
  try
    ...
    if LClipRect.Right > FLeftMargin.GetWidth then
    begin
      DrawRect := LClipRect;
    ...
    begin
      ...
      begin
        LLineCount := FTopLine + FVisibleLines;
        for i := FTopLine to LLineCount do
        begin
          DrawRect.Top := (i - FTopLine) * LineHeight;
...
procedure TBCBaseEditor.PaintCodeFolding(AClipRect: TRect; ALine: Integer);
var
  X, LHeight: Integer;
  LFoldRange: TBCEditorCodeFoldingRange;
begin
  ...
  begin
    X := AClipRect.Left + ((AClipRect.Right - AClipRect.Left) div 2);
    Canvas.MoveTo(X, AClipRect.Top);
    Canvas.LineTo(X, AClipRect.Top + ((AClipRect.Bottom - AClipRect.Top) div 2));
    Canvas.LineTo(AClipRect.Right, AClipRect.Top + ((AClipRect.Bottom - AClipRect.Top) div 2));
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        X := AClipRect.Left + ((AClipRect.Right - AClipRect.Left) div 2);
        Canvas.MoveTo(X, AClipRect.Top + (LineHeight div 2));
        Canvas.LineTo(X, AClipRect.Bottom);
      end;
  ...
  begin
    for i := FAllCodeFoldingRanges.AllCount - 1 downto 0 do
      if (FAllCodeFoldingRanges[i].IndentLevel > LDeepestLevel) and (CaretY >= FAllCodeFoldingRanges[i].FromLine) and
        (CaretY <= FAllCodeFoldingRanges[i].ToLine) then
        LDeepestLevel := FAllCodeFoldingRanges[i].IndentLevel;
    for i := 0 to FAllCodeFoldingRanges.AllCount - 1 do
      with FAllCodeFoldingRanges[i] do
      if not Collapsed and not ParentCollapsed and (FromLine < ALine) and (ToLine > ALine) then
      begin
        Y := ALineRect.Top;
        if AMinimap then
        ...
        begin
          ...
          if (LDeepestLevel = IndentLevel) and (CaretY >= FromLine) and (CaretY <= ToLine) then
...
begin
  ...
  if FCodeFolding.Visible then
  begin
    LCodeFoldingRect := AClipRect;
    if FCodeFolding.Visible then
      LCodeFoldingRect.Left := FLeftMargin.GetWidth
    else
      LCodeFoldingRect.Left := 0;
    LCodeFoldingRect.Right := FLeftMargin.GetWidth + FCodeFolding.GetWidth;
    Canvas.Brush.Color := CodeFolding.Colors.Background;
    Canvas.FillRect(LCodeFoldingRect);
    Canvas.Brush.Color := FLeftMargin.Color;
  end;
  ...
  begin
    ...
    try
      ...
      for i := 0 to Marks.Count - 1 do
        with Marks[i] do
          if Visible and (Line >= LFirstLine) and (Line <= LLastLine) then
          begin
            if not IsBookmark then
              LHasOtherMarks := True
            else
            begin
...
var
  ...
  begin
    ...
    if AMinimap then
      Result := Result + Pred(AColumn) * FMinimap.CharWidth
    else
      Result := Result + Pred(AColumn) * FTextDrawer.CharWidth;
  end;
  ...
  var
    ...
    LCountOfAvgGlyphs, LVisibleGlyphPart, LFillerCount, LNonFillerPosition: Integer;
    LCharWidth: Integer;
  const
  ...
  begin
    ...
    begin
      X := ColumnToXValue(AFirst, AMinimap);
      ...
      begin
        LFillerCount := 0;
        LNonFillerPosition := AFirst;
        while AToken[LNonFillerPosition] = BCEDITOR_FILLER_CHAR do
        begin
          inc(LFillerCount);
          inc(LNonFillerPosition);
        end;

        LCountOfAvgGlyphs := CeilOfIntDiv(fTextDrawer.TextWidth(AToken[LNonFillerPosition]), LCharWidth);
        LVisibleGlyphPart := LCountOfAvgGlyphs - LFillerCount;

        X := X - LCharWidth * (LVisibleGlyphPart - 1);

        LCharsToPaint := Min(ALast - AFirst + 1, ATokenLength - AFirst + 1);

        if GetWordWrap then
        while X + LCharWidth * LCharsToPaint > ClientWidth do
        begin
          Dec(LCharsToPaint);
  ...
  begin
    ...
    if WideChar(LLastChar) <= High(AnsiChar) then
    begin
      if GetCharABCWidths(Canvas.Handle, LLastChar, LLastChar, LCharInfo) then
      begin
        LRealCharWidth := LCharInfo.abcA + Integer(LCharInfo.abcB);
        if LCharInfo.abcC >= 0 then
          Inc(LRealCharWidth, LCharInfo.abcC);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          LTokenRect.Right := ColumnToXValue(LLineSelectionStart, AMinimap);
        ...
        LTokenRect.Right := ColumnToXValue(LSelectionEnd, AMinimap);
        with LTokenHelper do
        ...
        begin
          SetDrawingColors(False);
          LTokenRect.Right := ColumnToXValue(LLastColumn, AMinimap);
          with LTokenHelper do
    ...
    begin
      ...
      begin
        ...
        X2 := ColumnToXValue(LLineSelectionEnd, AMinimap);
...
begin
  ...
  try
    ...
    for i := 0 to FAllCodeFoldingRanges.AllCount - 1 do
    if Assigned(FAllCodeFoldingRanges[i]) then
      if FAllCodeFoldingRanges[i].Collapsed then
      begin
        LUncollapsedLineNumber := GetUncollapsedLineNumber(FAllCodeFoldingRanges[i].FromLine);
        if (LUncollapsedLineNumber <= LUndoItem.ChangeStartPosition.Line) and
          (LUncollapsedLineNumber + FAllCodeFoldingRanges[i].ToLine - FAllCodeFoldingRanges[i].FromLine >= LUndoItem.ChangeEndPosition.Line) then
        begin
          CodeFoldingUncollapseAll;
          Break;
        end;
      ...
      crDeleteCollapsedFold:
        begin
          ...
          begin
            for i := 0 to FAllCodeFoldingRanges.AllCount - 1 do
            begin
              if TBCEditorCodeFoldingRange(LUndoItem.ChangeData).FromLine <= FAllCodeFoldingRanges[i].FromLine then
              begin
                FAllCodeFoldingRanges.AllRanges.Insert(i, LUndoItem.ChangeData);
                FRedoList.AddChange(LUndoItem.ChangeReason, LUndoItem.ChangeStartPosition, LUndoItem.ChangeEndPosition, '',
                  LUndoItem.ChangeSelectionMode, FAllCodeFoldingRanges[i], i);
                Break;
              end;
...
begin
  ...
  begin
    ...
    l := Length(s);
    X := 0;
    for i := 1 to ATextPosition.Char - 1 do begin
      if (i <= l) and (s[i] = #9) then
        Inc(x, FTabs.Width {- (x mod FTabs.Width)})
      else
      if i <= l then
      begin
        LCountOfAvgGlyphs := CeilOfIntDiv(fTextDrawer.TextWidth(s[i]), fCharWidth);
...
begin
  ...
  for i := FAllCodeFoldingRanges.AllCount - 1 downto 0 do
    with FAllCodeFoldingRanges[i] do
      if (FoldRangeLevel = ALevel) and (not Collapsed) and (not ParentCollapsed) and Collapsable then
        CodeFoldingCollapse(FAllCodeFoldingRanges[i]);
    ...
    case ACommand of
      ...
      ecPageUp, ecSelectionPageUp, ecPageDown, ecSelectionPageDown:
        begin
         ...
         // if Command in [ecSelectionPageUp, ecSelectionPageDown] then
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  LVisualSpaceCount2 := 0; // Shut up Delphi compiler
      ...
      ecInsertLine, ecLineBreak:
        ...
        begin
          ...
          try
            ...
            DoLinesInserted(CaretY - InsertDelta, 1);
            SelectionBeginPosition := CaretPosition;
            SelectionEndPosition := CaretPosition;
            EnsureCursorPositionVisible;
          finally
...
begin
  ...
  for i := 0 to FAllCodeFoldingRanges.AllCount - 1 do
  with FAllCodeFoldingRanges[i] do
    if (CaretY = FromLine) and Collapsed then
      Exit;
  ...
  if not ParentCollapsed then
  begin
    if FromLine <= Length(LFoldRangeLookup) then
    begin
      LFoldRangeLookup[FromLine] := True;
  ...
  try
    ...
    try
      ...
      with FAllCodeFoldingRanges[i] do
      begin
        FromLine := LUncollapsedLinenumbersLookup[FromLine];
        ToLine := LUncollapsedLinenumbersLookup[ToLine];
      end;
      ...
      begin
        for i := 0 to FAllCodeFoldingRanges.AllCount - 1 do
          ...
          begin
            ...
            begin
              LTemporaryAllCodeFoldingRanges.AllRanges.Insert(j, FAllCodeFoldingRanges[i]);
              FAllCodeFoldingRanges[i] := nil; { destroy pointer }
              Break;
            end;
            if j = LTemporaryAllCodeFoldingRanges.AllCount - 1 then
            begin
              LTemporaryAllCodeFoldingRanges.AllRanges.Add(FAllCodeFoldingRanges[i]);
              FAllCodeFoldingRanges[i] := nil; { destroy pointer }
            end;
          end;
      end
